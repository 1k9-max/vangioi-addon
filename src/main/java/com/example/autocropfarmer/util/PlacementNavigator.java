...
import com.example.autocropfarmer.litematica.LitematicaCompat;
...
public class PlacementNavigator {
    ...
    /**
     * FIX (speed setting): di chuyen truc tiep bang setPosition() thay vi setVelocity(), tranh bi
     * vat ly bay cua vanilla (PlayerEntity#travel()) tinh lai/ghi de van toc moi tick khi khong co
     * phim WASD nao duoc nhan - day la nguyen nhan khien "speed" khong khop chinh xac voi gia tri
     * cau hinh. Xem giai thich chi tiet trong FlightController.
     */
    private boolean flyToward(ClientPlayerEntity player, Vec3d target, double speed) {
        ...
        Vec3d newPos = pos.add(diff.normalize().multiply(moveSpeed));
