...
public class AddonManager {
    ...
    public static void init() {
        // Meteor pseudo addon
        {
            MeteorClient.ADDON = new MeteorAddon() {
                ...
                public String getCommit() {
                    String commit = MeteorClient.MOD_META.containsCustomValue(MeteorClient.MOD_ID + ":commit")
                        ? MeteorClient.MOD_META.getCustomValue(MeteorClient.MOD_ID + ":commit").getAsString()
                        : "";
        ...
        for (EntrypointContainer<MeteorAddon> entrypoint : FabricLoader.getInstance().getEntrypointContainers("meteor", MeteorAddon.class)) {
            ...
            if (metadata.containsCustomValue(metadata.getId() + ":color")) {
                addon.color.parse(metadata.getCustomValue(metadata.getId() + ":color").getAsString());
            }
